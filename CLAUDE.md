...
13. `BreathingService` владеет настоящим `remainingAir` только во время высотной гипоксии, начиная с config-Y включительно. Authoritative-карта зеркалится в игрока каждый тик и имеет приоритет над расовым дыханием Merman на этой высоте.
...
Тесты покрывают round-trip YAML, цикл высотной гипоксии и сухопутный цикл воздуха Merman. Перед релизом также проверить запуск на чистом Paper 1.21.11, `/race set|clear|reload`, миграцию старых предметов и повторный старт с установленным датапаком.
