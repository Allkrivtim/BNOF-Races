...
- `BnofRacesPlugin` собирает зависимости, listeners, правила и общий heartbeat с базой в один серверный тик.
...
- `Ability`, `TickAbility`, `PassiveEffectAbility`, `ConditionalPassiveEffectAbility` и `EventAbilities` — публичные контракты расширения.
...
5. Пассивные эффекты бесконечны и обновляются при assignment/join/respawn/world-change/wake-up и раз в 6000 тиков. Молоко, `/effect clear` и Blessing снимают их до следующего refresh; conditional-пассивы дополнительно возвращаются на false→true переходе и по умолчанию не снимаются при true→false. Утомление Archangel явно включает очистку при выходе из высотного условия.
6. Расовый предмет валиден только при полном наборе owner/race/item/schema. Текущая schema — 2; старые `oneframe:*` предметы распознаются и пересоздаются. Reconciliation учитывает предмет на курсоре, а transfer guard не блокирует перестановку внутри собственного инвентаря.
...
Тесты покрывают round-trip YAML, цикл высотной гипоксии и сухопутный цикл воздуха Merman. Перед релизом также проверить lifecycle/пятиминутный refresh бесконечных пассивов, выборочное Blessing, конфиг высоты Archangel, `/race set|clear|reload`, миграцию старых предметов и повторный старт с установленным датапаком.
