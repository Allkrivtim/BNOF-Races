...
Выше `altitude-hypoxia-y` гипоксия использует настоящий запас воздуха: появляется и расходуется шкала пузырьков, а урон начинается после её исчерпания. Исключение — расы с флагом `ALTITUDE_HYPOXIA`.

Пассивные эффекты намеренно накладываются только при назначении расы, входе и респавне. Молоко и `/effect clear` снимают их до следующего такого события.
