...
- Высотная гипоксия реализована `BreathingService`: начиная с config-Y включительно все неосвобождённые расы расходуют настоящий ванильный `remainingAir` от максимума до drowning-порога, видят шкалу пузырьков и затем получают обычный урон. Отдельная authoritative-карта не даёт ванильному восстановлению сбросить отсчёт между тиками. На этой высоте сервис имеет приоритет над расовым дыханием Merman; `ALTITUDE_HYPOXIA` освобождает расу от правила — его несут Archangel и Seraphim (`AngelShared.EXEMPTIONS`).
