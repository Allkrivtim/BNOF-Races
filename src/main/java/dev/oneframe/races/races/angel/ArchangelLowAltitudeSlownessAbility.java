...
/**
 * Slowness II only below Y=200 - a grounded-flight balance: no penalty while actually up in the
...
public final class ArchangelLowAltitudeSlownessAbility implements TickAbility {
    ...
    public String description() {
        return "Ниже Y=200 получает Slowness II.";
    }
    ...
    public void tick(Player player, AbilityContext ctx) {
        if (player.getLocation().getY() < THRESHOLD_Y) {
            player.addPotionEffect(new PotionEffect(
                    PotionEffectType.SLOWNESS, DURATION_TICKS, SLOWNESS_AMPLIFIER, true, false));
        }
    }
}
